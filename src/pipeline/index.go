...
func (proc *MSAconverter) Run() {
	...
	for i, msaFile := range proc.input {
		...
		// create a GrootGraph
		grootGraph, err := graph.CreateGrootGraph(newGFA, i)
		if err != nil {
			misc.ErrorCheck(err)
		}
...
func (proc *GraphSketcher) Run() {
	...
	for newGraph := range proc.input {
		...
		// store the graph
		graphStore[newGraph.GraphID] = newGraph
