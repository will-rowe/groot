...
/*
  A process to check FASTA entries
*/
type MsaChecker struct {
	process
	Input        chan seqio.FASTAentry
	Output       chan seqio.FASTAentry
	MinSeqLength int
}

func NewMsaChecker() *MsaChecker {
	return &MsaChecker{Output: make(chan seqio.FASTAentry, BUFFERSIZE)}
}

func (proc *MsaChecker) Run() {
	defer close(proc.Output)
	duplicateHeaders := make(map[string]struct{})
	for reference := range proc.Input {
		// ignore the consensus
		if string(reference.ID) == "consensus" {
			continue
		}
		...
		if len(reference.Seq) < proc.MinSeqLength {
			log.Printf("\tWARNING: MSA entry is shorter than the supplied read length (used as window size): %v", string(reference.ID))
			reference.Seq = nil
		}
		// convert all bases in reference sequence to upper case and check for non-AGCTN bases
		if err := reference.BaseCheck(); err != nil {
			log.Printf("\tWARNING: MSA entry has %v: %v", err, string(reference.ID))
			reference.Seq = nil
		}
		// send reference via channel
		proc.Output <- reference
	}
}
